...
{
    public class DeliveryEventHandler : IScanEventHandler
    {
        public bool CanHandle(string eventType) => eventType.Equals("DELIVERY", StringComparison.OrdinalIgnoreCase);

        public Task HandleAsync(ScanEvent scanEvent, ParcelState state)
        {
            state.DeliveryTime = scanEvent.CreatedDateTimeUtc;
